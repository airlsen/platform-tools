      ...
      <ul class="nav">
        <template v-for="item in routes">
            <router-link tag="li"  class="nav-item nav-dropdown"  v-if="!item.hidden&&item.children&&item.children.length>0" :to="item.path+''+item.children[0].path" disabled>
                <div class="nav-link nav-dropdown-toggle" style=";font-size: 13px;font-weight: 600;" @click="handleClick"><Icon :type="item.icon" color="white"/>{{ item.name}}  </div>
                   <ul class="nav-dropdown-items">
                     <li class="nav-item" v-for="child in item.children" v-if='!child.hidden' @click="addActive" style="padding-left: 15px;font-size: 12px">
        ...
        <li class="nav-item" v-if="!item.hidden&&!item.children" style="font-size: 13px">
          <router-link  :to="item.path" class="nav-link" exact style="font-weight: 600;"><Icon :type="item.icon" color="white" />{{ item.name}}
          </router-link>
        </li>
